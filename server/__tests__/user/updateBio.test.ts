...
describe('Update Bio Unit Testing', () => {
    ...
    describe('Failed request cause no access_token', () => {
        ...
        it('Should return error message and status code 401', async () => {
            ...
            const { statusCode, body: { error, data } } = await fetch.patch(`${BASE_URL}/bio`);
    ...
    describe('Failed request cause empty body', () => {
        ...
        it('Should return error message and status code 406', async () => {
            ...
            const { statusCode, body: { error } } = await fetch.patch(`${BASE_URL}/bio`).send({})
            ...
            expect(error).toEqual([
                {
                    field: 'newBio',
                    message: 'Please provide the new bio!'
                }
            ]);
        })
    ...
    describe('Success request', () => {
        ...
        it('Should return new bio', async () => {

            const newBio = 'I am a Software Enginerring!'

            const { statusCode, body: { data } } = await fetch.patch(`${BASE_URL}/bio`).send({ newBio })
                .set('x-access-token', tokens.access_token).set('x-refresh-token', tokens.refresh_token);
