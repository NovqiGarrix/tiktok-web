...
describe('UnLikePost Route Test', () => {
    ...
    describe('Failed request for no access_token', () => {
        ...
        it('Should return error message and status code 401', async () => {
            ...
            const { statusCode, body: { error, data } } = await fetch.get(`${BASE_URL}/${post._id}/like`);
    ...
    describe('Success request', () => {
        ...
        it('Should return unLikedPost', async () => {
            ...
            // First, like the post
            const { statusCode: likeStatusCode } = await fetch.post(`${BASE_URL}/${post._id}/like`)
                .set('x-access-token', tokens.access_token)
                .set('x-refresh-token', tokens.refresh_token);

            expect(likeStatusCode).toBe(200);

            // Then, unlike it
            const { statusCode, body: { data: unLikedPost } } = await fetch.patch(`${BASE_URL}/${post._id}/like`)
                .set('x-access-token', tokens.access_token)
                .set('x-refresh-token', tokens.refresh_token);

            expect(statusCode).toBe(200);
            expect(unLikedPost).not.toBeNull();
            expect(unLikedPost.post.likes).toBe(0);

            post = unLikedPost.post
        ...
        it('Should return 0 if the post likes < 0 and user try to unlike the post', async () => {
